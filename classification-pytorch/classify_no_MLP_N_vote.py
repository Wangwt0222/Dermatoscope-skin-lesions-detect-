...
def getErrClassification_mgpu(indices, batchSize, numGPUs, device, train=False):
    ...
    predictions_mc = np.zeros([len(indices),7,multiCropEval])
    targets_mc = np.zeros([len(indices),7,multiCropEval])   
...
#     summary(model.to(device),(3,224,224))
    ...
    #模型名字
    run_name = 'resnet101_no_MLP_N36_vote'
