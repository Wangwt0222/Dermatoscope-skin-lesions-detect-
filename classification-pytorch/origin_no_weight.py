...
#         if (int(math.ceil(len(indices) / batchSize )) - 1 == i) and indices is not 'trainInd':
#             bSize = len(indices) - batchSize * i
#         else:
#             bSize = batchSize
#         print(i)
#         print('bSize:', bSize)
#         print('labels.data:', labels.data.cpu().numpy())
...
#         for k in range(len(numGPUs)):
#             loss_all[(i*len(numGPUs)+k)*bSize:(i*len(numGPUs)+k+1)*bSize] = loss
#             predictions[(i*len(numGPUs)+k)*bSize:(i*len(numGPUs)+k+1)*bSize,:] = preds
#             tar_not_one_hot = labels.data.cpu().numpy()
#             tar = np.zeros((tar_not_one_hot.shape[0], 7))
#             tar[np.arange(tar_not_one_hot.shape[0]), tar_not_one_hot] = 1
#             targets[(i*len(numGPUs)+k)*bSize:(i*len(numGPUs)+k+1)*bSize,:] = tar
    # Accuarcy正确率
    ...
    return np.mean(loss_all), acc, sensitivity, specificity, conf, f1, roc_auc, wacc, predictions, targets
...
# model = models.senet154(pretrained=True)
#修改最后分类层
...
if __name__ == "__main__":
    ...
    #模型名字
    run_name = 'resnet101_no_multicrop_weight'
    log_filename = get_log_filename(run_name)
...
#             print("[%d|%d] loss:%f" % (j, len(trainloader), loss.mean()))
        if step % 5 == 0 or step == 1:
            ...
            model.eval()
            loss, accuracy, sensitivity, specificity, conf_matrix, f1, Roc_Auc, waccuracy, predictions, targets = \
            ...
            if eval_metric < valBest:
                ...
                if os.path.isfile(get_run_dir(run_name) + '/checkpoint_best-' + str(oldBestInd) + '.pt'):
                    os.remove(get_run_dir(run_name) + '/checkpoint_best-' + str(oldBestInd) + '.pt')
                # Save currently best model
                state = {'epoch': step, 'state_dict': model.state_dict(),
                         'optimizer': optimizer.state_dict()}
                torch.save(state, get_run_dir(run_name) + '/checkpoint_best-' + str(step) + '.pt')

                # If its not better, just save it delete the last checkpoint if it is not current best one
            # Save current model
            state = {'epoch': step, 'state_dict': model.state_dict(),
                     'optimizer': optimizer.state_dict()}
            torch.save(state, get_run_dir(run_name) + '/checkpoint-' + str(step) + '.pt')
            # Delete last one
            if step == display_step:
                lastInd = 1
            else:
                lastInd = step - display_step
            if os.path.isfile(get_run_dir(run_name) + '/checkpoint-' + str(lastInd) + '.pt'):
                os.remove(get_run_dir(run_name) + '/checkpoint-' + str(lastInd) + '.pt')
            # Print
            print("\n")
            print('Fold: %d Epoch: %d/%d (%d h %d m %d s)' % (
            0, step, training_steps, int(duration / 3600), int(np.mod(duration, 3600) / 60),
            int(np.mod(np.mod(duration, 3600), 60))) + time.strftime("%d.%m.-%H:%M:%S", time.localtime()))
            print("Loss on ", eval_set, "set: ", round(Decimal(loss),4), " Accuracy: ", round(Decimal(accuracy),4), " F1: ", round(Decimal(f1),4))
