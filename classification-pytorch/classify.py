...
#                 color_distort,
                ...
                transforms.Normalize(torch.from_numpy(self.setMean).float(),
                                     torch.from_numpy(np.array([0.229, 0.224, 0.225])).float())
            ])
            ...
            # Set up transforms
            self.norm = transforms.Normalize(torch.from_numpy(self.setMean).float(),torch.from_numpy(np.array([0.229, 0.224, 0.225])).float())
...
#     model.classifier = nn.Linear(num_ftrs, 7)
    ...
    if __name__ == "__main__":
        #模型保存位置
        model_data_dir = "/home/deeplearning/wwt/weight_test/2019_7_25/model_data/"
        #模型名字
        run_name = 'resnet101_test'
